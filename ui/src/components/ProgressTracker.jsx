...
const ProgressTracker = ({ progress, message }) => {
  ...
  const getSteps = () => {
    ...
    if (msg.includes('audio') || msg.includes('generating podcast') || msg.includes('generating audify') || msg.includes('creating podcast') || msg.includes('creating audify')) {
        ...
        <div className="mt-8 space-y-3 text-left max-w-md mx-auto">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={index}
                className={`flex items-center transition-colors ${isCompleted ? 'text-green-600' :
              ...
              >
                {isCompleted ? (
                  ...
                  <div className={`w-2 h-2 rounded-full mr-3 flex-shrink-0 ${isCompleted ? 'bg-green-600' :
