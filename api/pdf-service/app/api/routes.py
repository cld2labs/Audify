...
from app.core.doc_extractor import DocExtractor
...
async def extract_text(
    ...
    try:
        ...
        if not (filename.endswith('.pdf') or filename.endswith('.doc') or filename.endswith('.docx')):
            raise HTTPException(
                status_code=400,
                detail="Supported formats: PDF, DOC, DOCX"
            )
        ...
        extracted_result = {"text": "", "metadata": {}, "method": "unknown"}
        
        # Dispatch based on extension
        if filename.endswith('.doc'):
            logger.info("Processing legacy DOC file...")
            extracted_result = doc_extractor.extract_doc(content)
        elif filename.endswith('.docx'):
            logger.info("Processing DOCX file...")
            extracted_result = doc_extractor.extract_docx(content)
        else:
        ...
        # Use a consistent variable name for cleaning
