...
STORY_TELLER_PROMPT = """Create a narrative-driven podcast script. Use:
- Strong storytelling elements (setting the scene, character development, conflict, resolution)
- Emotional connection and vivid descriptions
- "Show, don't just tell" approach
- Host guides the listener through the "journey"
- Great for case studies, history, or biographies
Target audience: Listeners who love stories and narratives"""

DEBATE_PROMPT = """Create a dynamic, debate-style podcast script. Use:
- Host plays "Devil's Advocate" and challenges the guest's points
- Respectful but firm pushback on assumptions
- Critical analysis of the topic from multiple angles
- Dynamic back-and-forth energy
- Highlight controversies or differing viewpoints
Target audience: Listeners who want to hear arguments tested"""

QUICK_BRIEF_PROMPT = """Create a fast-paced, high-density summary script. Use:
- Zero "fluff" or small talk
- Immediate focus on the "Big 3" takeaways
- Concise, punchy sentences
- rapid-fire exchange of information
- Summarize key points efficiently
Target audience: Busy professionals wanting a quick update"""

ELI5_PROMPT = """Create a simple, easy-to-understand podcast script (Explain Like I'm 5). Use:
- Crystal clear analogies for every complex concept
- NO jargon (or define it immediately if unavoidable)
- Relatable, everyday examples (e.g., "Think of it like a pizza...")
- Enthusiastic and encouraging tone
- Break everything down to first principles
Target audience: Students or complete beginners"""

AUDIENCE_PROMPTS = {
    "general": "Target audience: General listeners. Specific instructions: Use balanced language, explain complex terms briefly.",
    "beginner": "Target audience: Beginners/Students. Specific instructions: Use simple language, heavy use of analogies, avoid all jargon.",
    "expert": "Target audience: Industry Experts. Specific instructions: Use technical terminology, skip basic definitions, focus on advanced concepts."
}
...
def get_tone_prompt(tone: str) -> str:
    ...
    tone_prompts = {
        ...
        "deep_dive": CONVERSATIONAL_TONE_PROMPT, # Alias for default
        "educational": EDUCATIONAL_TONE_PROMPT,
        "professional": PROFESSIONAL_TONE_PROMPT,
        "storyteller": STORY_TELLER_PROMPT,
        "debate": DEBATE_PROMPT,
        "quick_brief": QUICK_BRIEF_PROMPT,
        "eli5": ELI5_PROMPT,
    }
    return tone_prompts.get(tone.lower(), CONVERSATIONAL_TONE_PROMPT)

def get_audience_prompt(audience: str) -> str:
    """Get audience-specific guidance"""
    return AUDIENCE_PROMPTS.get(audience.lower(), AUDIENCE_PROMPTS["general"])
...
def get_user_prompt(
    ...
    return USER_PROMPT_TEMPLATE.format(
        ...
        audience_instruction=get_audience_prompt(target_audience),
